var OmcTrucks4 = {
    ...
    handleGridEvent:function (com, grid, json) {
        if (com == 'New') {
            OmcTrucks4.objGrid.flexBeginAdd();
        }
        else if (com == 'Edit') {
            //var row = jLib.getSelectedRows(grid);
            var row = FlexObject.getSelectedRows(grid);
            OmcTrucks4.objGrid.flexBeginEdit(row[0]);
        }
        else if (com == 'Save') {
            OmcTrucks4.objGrid.flexSaveChanges();
        }
        else if (com == 'Cancel') {
            OmcTrucks4.objGrid.flexCancel();
        }
        else if (com == 'Delete') {
            if (FlexObject.rowSelectedCheck(OmcTrucks4.objGrid,grid,1)) {
                OmcTrucks4.delete_(grid);
            }
        }
        else if (com == 'Export All') {
            var url = $("#export4_url").val();
