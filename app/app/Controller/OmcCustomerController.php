...
class OmcCustomerController extends OmcCustomerAppController
{
	...
	function dashboard()
	{
		...
		$dsl_bar_data = $this->OmcCustomerReport->getDailySalesLiters($company_profile['id'], $company_profile['omc_id'], $date);
	...
	function cm_customer_sales_ledger($type = 'get') {
		...
		$customer_name_lists_data = $this->get_credit_customers();
		$customer_name_lists = array( array('name'=>'All','value'=>0));
		foreach($customer_name_lists_data as $obj){
			$customer_name_lists[] = array(
				'name'=>$obj['name'],
				'value'=>$obj['id']
			);
		}
	...
	function export_customer_data() {
		...
		if ($this->request->is('post')) {
			$data_source = $this->request->data['Export']['data_source'];
			$export_start_dt = $this->request->data['Export']['export_start_dt'];
			$export_end_dt = $this->request->data['Export']['export_end_dt'];

			if($data_source === 'credit_approval') {
				$export_data =$this->CustomerCredit->export_credit_approval($company_profile['id'], $export_start_dt, $export_end_dt);;
			}
			if($data_source === 'credit_request_approval') {
				$export_data =$this->CustomerCredit->export_credit_request_approval($company_profile['id'], $export_start_dt, $export_end_dt);
			}
			if($data_source === 'credit_sales') {
				$export_data =$this->CustomerCredit->export_credit_sales($company_profile['id'], $export_start_dt, $export_end_dt);
			}
			if($data_source === 'credit_payment') {
				$export_data =$this->CustomerCreditPayment->export_credit_payment($company_profile['id'], $export_start_dt, $export_end_dt);
			}

			if ($export_data['data']) {
				$filename = $company_profile['name'] . " - ".$export_data['filename'];
				$download = true;
				$res = $this->convertToExcel($export_data['header'], $export_data['data'], $filename);
				$objPHPExcel = $res['excel_obj'];
				$filename = $res['filename'];
			}
		}
