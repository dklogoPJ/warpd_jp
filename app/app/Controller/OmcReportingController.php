...
class OmcReportingController extends OmcAppController
{
    ...
    function margin_analysis (){
        ...
        $report_types = array(
            ...
            'margin_customer'=>'Margin Analysis On Customers',
            'margin_customer_segment'=>'Margin Analysis On Customer Segment',
            'margin_product'=>'Margin Analysis On Product',
            'margin_bdc'=>'Margin Analysis On BDC'
        );

        if($this->request->is('post')){
            //$start_dt = $this->covertDate($this->request->data['Query']['start_dt'],'mysql');
            //$end_dt = $this->covertDate($this->request->data['Query']['end_dt'],'mysql');
            $report_type = $this->request->data['Query']['report_type'];
            $year = $this->request->data['Query']['year'];
            $month = $this->request->data['Query']['month'];
        }

        $g_data =  $this->get_margin_analysis($year,$month,$report_type);
        $graph_title = $report_types[$report_type].' '.$month.'-'.$year;
        $table_title = $report_types[$report_type].' '.$month.'-'.$year;
        $month_list = $this->getMonths();
        $controller = $this;

        $this->set(compact('controller','g_data','month_list','report_types','report_type','print_title','table_title','graph_title','year','month'));
    }

    function print_export_margin_analysis(){
        $download = false;
        $company_profile = $this->global_company;
        $report_types = array(
            'margin_default'=>'Margin Analysis On Transactions',
        ...
        $media_type = $_POST['data_type'];
        $report_type = $_POST['data_report_type'];
        $year = $_POST['data_year'];
        $month = $_POST['data_month'];
        $g_data =  $this->get_margin_analysis($year,$month,$report_type);
        $grid_data = $g_data['table'];
        $t_head = $grid_data['t_head'];
        $t_body_data = $list_data = $grid_data['t_body'];

        $export_title = $report_types[$report_type].' '.$month.'-'.$year;
        $table_title = $report_types[$report_type].' '.$month.'-'.$year;
        $graph_title = $report_types[$report_type].' '.$month.'-'.$year;
