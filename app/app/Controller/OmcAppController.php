...
class OmcAppController extends AppController
{
    ...
    function getOmcPriceChangeData(){
        ...
        $price_change = $PriceChange->getPriceQuotesData($company_profile['id']);
        return $price_change;
    }

    function get_customer_category(){
        $lists = array(
            array('id' => 'Commercial', 'name' => 'Commercial'),
            array('id' => 'Company Own', 'name' => 'Company Own'),
            array('id' => 'Dealer Own', 'name' => 'Dealer Own')
        );
        return $lists;
    }

    function get_margin_analysis($year,$month,$report_type){
        $company_profile = $this->global_company;
        $OmcMargin = ClassRegistry::init('OmcMargin');
        $data = $OmcMargin->margin_analysis($company_profile['id'],$year,$month,$report_type);
        return $data;
    }
