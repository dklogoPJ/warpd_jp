var OmcTrucks3 = {
    ...
    handleGridEvent:function (com, grid, json) {
        if (com == 'New') {
            OmcTrucks3.objGrid.flexBeginAdd();
        }
        else if (com == 'Edit') {
            //var row = jLib.getSelectedRows(grid);
            var row = FlexObject.getSelectedRows(grid);
            OmcTrucks3.objGrid.flexBeginEdit(row[0]);
        }
        else if (com == 'Save') {
            OmcTrucks3.objGrid.flexSaveChanges();
        }
        else if (com == 'Cancel') {
            OmcTrucks3.objGrid.flexCancel();
        }
        else if (com == 'Delete') {
            if (FlexObject.rowSelectedCheck(OmcTrucks3.objGrid,grid,1)) {
                OmcTrucks3.delete_(grid);
            }
        }
        else if (com == 'Export All') {
            var url = $("#export3_url").val();
