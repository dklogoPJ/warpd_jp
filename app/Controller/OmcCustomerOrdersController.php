        ...
        if ($this->request->is('ajax')) {
            ...
            switch ($type) {
                case 'get' :
                    ...
                    if ($data_table) {
                        ...
                        foreach ($data_table as $obj) {
                           ...
                           // $received_quantity = isset($obj['OmcCustomerOrder']['received_quantity']) ? $this->formatNumber($obj['OmcCustomerOrder']['received_quantity'], 'number', 0) : '';
                            //$variance_received_qty = isset($obj['OmcCustomerOrder']['variance_received_qty']) ? $this->formatNumber($obj['OmcCustomerOrder']['variance_received_qty'], 'number', 0) : '';

                            $invoice_date = isset($obj['OmcCustomerOrder']['invoice_date']) ? $this->covertDate($obj['OmcCustomerOrder']['invoice_date'], 'mysql_flip') : '';
                            ...
                            $return_arr[] = array(
                                ...
                                'cell' => array(
                                    $obj['OmcCustomerOrder']['id'],
                                    $invoice_date,
                                    $obj['OmcCustomerOrder']['invoice_no'],
                                    $obj['OmcCustomerOrder']['id'],
                                    $obj['ProductType']['name'],
                                    $obj['OmcCustomerOrder']['transporter'],
                                    $obj['OmcCustomerOrder']['volume_depot'],
                                    $obj['OmcCustomerOrder']['product_density_depot'],
                                    $obj['OmcCustomerOrder']['temp_20'],
                                    $obj['OmcCustomerOrder']['product_temp_depot'],
                                    $obj['OmcCustomerOrder']['product_coeff'],
                                    $obj['OmcCustomerOrder']['temp_coeff_1'],
                                    $obj['OmcCustomerOrder']['product_temp_station'],
                                    $obj['OmcCustomerOrder']['vol_15'],
                                    $obj['OmcCustomerOrder']['temp_coeff_2'],
                                    $obj['OmcCustomerOrder']['temp_vol_station'],
                                    $obj['OmcCustomerOrder']['received_quantity'],
                                    $obj['OmcCustomerOrder']['variance_depot'],
                                    $obj['OmcCustomerOrder']['variance_received_qty']
                                )
                            );
                        }
                        return json_encode(array('success' => true, 'total' => $total_records, 'page' => $page, 'rows' => $return_arr));
                ...
                case 'save' :
                    ...
                    $_POST['invoice_date'] = $this->covertDate($_POST['invoice_date'], 'mysql') . ' ' . date('H:i:s');

                    $data = array('OmcCustomerOrder' => $_POST);

                    if ($_POST['id'] == 0) {
                        $data['OmcCustomerOrder']['created_by'] = $authUser['id'];
                    } else {
                        $data['OmcCustomerOrder']['modified_by'] = $authUser['id'];
                    }

                    $data['OmcCustomerOrder']['omc_customer_id'] = $company_profile['id'];

                    if ($this->OmcCustomerOrder->save($this->sanitize($data))) {
                        $temperature_compensation_id = $this->OmcCustomerOrder->id;
                ...
                case 'delete':
                    $ids = $_POST['ids'];
                    $modObj = ClassRegistry::init('OmcCustomerOrders');
                    $result = $modObj->updateAll(
                        $this->sanitize(array('OmcCustomerOrders.deleted' => "'y'")),
                        $this->sanitize(array('OmcCustomerOrders.id' => $ids))
                    );
                    if ($result) {
                        echo json_encode(array('code' => 0, 'msg' => 'Data Deleted!'));
                    } else {
